...
class AlchemyAPI {
	...
	*/ 
	public function AlchemyAPI() {
		...
		if(!$key)
		{
		    //If the key is not found in VCAP_SERVICES, try USER_DEFINED variables
			$key = getenv('alchemyapikey');
		}
		
		if (!$key) {
			//Keys should not be blank
			echo 'The apikey appears to be blank, please make sure that you input the apikey while creating the AlchemyAPI service', PHP_EOL;
		...
		if (strlen($key) != 40) {
			...
			echo 'It appears that the apikey is invalid. Please make sure it is the correct one.', PHP_EOL;
			exit(1);
		}
	...
	*/
	...
	public function face_detection($flavor, $image, $options) {
		...
		if (!array_key_exists($flavor, $this->_ENDPOINTS['face_detection'])) {
			return array('status'=>'ERROR','statusInfo'=>'Face detection for ' . $flavor . ' not available');
		}
		...
		if($flavor=='url'){
			...
			return $this->analyzeImage($this->_ENDPOINTS['face_detection'][$flavor], $options, $image);	
		}
	}
